...
const AddToCartButton = ({
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      updateHeaderdetails({ user: authData.user });
      setIsLoggedIn(true);
  ...
  return (
    <button
      ...
      className={`w-full sm:w-auto px-3 py-2 rounded-md shadow-md transition duration-300 text-md flex items-center justify-center gap-x-3
        ${isOutOfStock
          ? 'bg-gray-400 cursor-not-allowed text-white'
          : isLoading
            ? 'bg-red-600 cursor-not-allowed opacity-75 text-white'
            : cartSuccess
              ? 'bg-green-500 text-white hover:bg-green-600'
              : 'bg-red-600 text-white hover:bg-yellow-400 hover:text-red-700'}
