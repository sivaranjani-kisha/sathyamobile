...
const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        setCategories(parentCategories); // This sets the categories for the filter
  ...
  const filteredProducts = selectedCategory
    ? products.filter((product) => product.category === selectedCategory.category_slug)
    : products;
            ...
            <div className="grid grid-cols-4 gap-6">
              ...
              {featuredCategory && (
                <div className="col-span-1 bg-green-600 text-white p-6 rounded-lg flex flex-col justify-center items-center">
                  <h3 className="text-3xl font-bold">{featuredCategory.category_name}</h3>
                  {featuredCategory.image && (
                    <Image 
                      src={featuredCategory.image} 
                      alt={featuredCategory.category_name} 
                      width={200} 
                      height={160} 
              ...
              <div className="col-span-3 flex space-x-6 overflow-x-auto pb-4">
                {dealCategories.map((category, index) => {
                  const badgeTypes = [
                    { type: "Sale 50%", color: "bg-red-500" },
                    { type: "Best Sale", color: "bg-blue-500" },
                    { type: "New", color: "bg-orange-500" }
                  ];
                  ...
                  return (
                    <motion.div
                      key={category._id}
                      whileHover={{ y: -5 }}
                      className="relative min-w-[250px] p-4 bg-white rounded-lg shadow-md hover:shadow-lg transition"
                    >
                   
                      

                      <div className="absolute top-3 right-3 z-10">
    <ProductCard productId={category._id} wishliststatus={category.wishlist} />
  </div>
                      ...
                      {category.image && (
                        <Image
                          ...
                          width={200}
                          height={160}
                          className="w-full h-40 object-contain mb-20"
                        />
                      )}
                      <h3 className="text-sm font-semibold mt-2">{category.category_name}</h3>
                      <p className="text-green-600 text-sm">
                        $14.99 <span className="line-through text-gray-400">$28.99</span>
                      </p>
                      <motion.button
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className="w-full mt-3 bg-green-600 text-white py-2 rounded flex items-center justify-center gap-2"
                      >
