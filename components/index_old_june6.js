...
>
  ...
  <Image 
    ...
    fill
    priority 
    className="object-cover w-full h-full" // object-cover fixes squish
    sizes="100vw"
  />
</div>
  ...
  <div className="grid grid-cols-2 gap-4 max-w-screen-xl mx-auto lg:flex lg:flex-nowrap lg:justify-center lg:gap-6">
    {features.map((feature, index) => (
      <div
        key={index}
        className="flex items-center lg:flex-row flex-col text-center lg:text-left flex-1 min-w-0 p-6 rounded-xl shadow-md bg-gradient-to-br from-[#deb9b9] to-[#73a0e0] h-full"
      >
        <div className="bg-white text-blue-600 p-3 rounded-full text-2xl flex items-center justify-center mb-3 lg:mb-0">
          {feature.icon}
        </div>
                    ...
                    {offerProducts.length > 0 && offerProducts.length <= 2 && (
                        ...
                        {offerProducts.map((product, index) => (
                            ...
                            <div className="flex items-center">
                                ...
                                <Link href={`/product/${product.slug}`} className={`block ${navigating ? 'pointer-events-none' : ''}`} onClick={(e) => {
                        e.preventDefault();
