...
export default function CartComponent() {
    ...
    useEffect(() => {
        const fetchCartData = async () => {
            ...
            //    alert(JSON.stringify(data, null, 2));
                ...
                // alert(JSON.stringify(data.cart, null, 2));
  ...
  item.warranty > 0 && (
    <div key={index} className="flex justify-between text-gray-600">
      <span>Warranty</span>
      <span className="font-semibold text-gray-900">₹{item.warranty}</span>
    </div>
  )
))}

    {/* Extended Warranty Section - Only show if value exists */}
    {cartData.items && cartData.items.map((item, index) => (
  item.extendedWarranty > 0 && (
    <div key={index} className="flex justify-between text-gray-600">
      <span>Extended Warranty</span>
      <span className="font-semibold text-gray-900">₹{item.extendedWarranty}</span>
    </div>
  )
))}
