...
import { sendRecoveryOtp } from "@/lib/api";
...
//const [recoveryMethod, setRecoveryMethod] = useState(null); // 'email' or 'mobile'
...
const handleForgotPassword = async (e) => {
  ...
  // Validate input
  const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recoveryInput);
  const isMobile = /^[0-9]{10}$/.test(recoveryInput); // Assuming 10-digit mobile numbers
  
  if (!isEmail && !isMobile) {
    setFormError('Please enter a valid email or 10-digit mobile number');
    return;
  }
  ...
  try {
    ...
    // API call to send OTP - replace with your actual API call
    ...
    setError(err.message || 
            (isEmail ? 'Failed to send OTP to email' : 'Failed to send OTP to mobile'));
  } finally {
...
const handleOtpVerification = async (e) => {
  ...
  try {
    // API call to verify OTP - replace with your actual API call
    ...
    // You can now show password reset form
...
const handleResendOtp = async () => {
  ...
  try {
    ...
    alert(`New OTP sent to your ${recoveryMethod}`);
  } catch (err) {
...
//     // const response = await api.verifyRecoveryOtp({ 
//     //   [recoveryMethod]: recoveryInput, 
//     //   otp 
//     // });
    
//     // For demo purposes, we'll just simulate this
//     console.log(`Verifying OTP for ${recoveryMethod}:`, recoveryInput, otp);
    
//     // Handle successful verification - you might want to show a password reset form
//     alert('OTP verified successfully! Please reset your password.');
//     // You would typically set another state here to show the password reset form
//   } catch (err) {
    ...
    <div className="bg-white rounded-lg p-8 w-96 max-w-full relative">
      <button 
        onClick={() => {
          ...
          setFormError('');
          setError('');
          setActiveTab(activeTab === 'forgot' ? 'login' : activeTab);
          setShowOtpVerification(false);
        }}
      ...
      {!showOtpVerification ? (
        activeTab !== 'forgot' ? (
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <button
                  ...
                  className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 disabled:bg-gray-400 transition-colors duration-200"
                >
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-semibold mb-4">Password assistance</h2>
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600 mb-4">
            We've sent a 6-digit OTP to your {recoveryMethod === 'email' ? 'email' : 'mobile number'}. Please enter it below.
          </p>
