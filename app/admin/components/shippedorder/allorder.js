...
useEffect(() => {
  const fetchOrders = async () => {
    ...
    const homeOrders = data.filter(order => order.order_status === 'shipped');
  ...
  return (
     <div className="container mx-auto">
      ...
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-2xl font-bold">Shipped order</h2>
  ...
  paginatedOrders.map((o, i) => (
    <tr key={i} className="border-t">
      <td className="p-2 border flex justify-center items-center">
        <button
          onClick={() => router.push(`/admin/homedelivery/${o._id}`)}
