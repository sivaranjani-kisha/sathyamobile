...
const handleDeleteBanner = async () => {
 ...
 if (createdDateFilter.startDate && createdDateFilter.endDate) {
  url += `&createdStartDate=${createdDateFilter.startDate.toISOString()}`;
  url += `&createdEndDate=${createdDateFilter.endDate.toISOString()}`;
}
 ...
 const handleDateChange = ({ startDate, endDate }) => {
  ...
  if (start && end && start.toDateString() === end.toDateString()) {
    ...
    setCreatedDateFilter({ startDate: start, endDate: end });
  } else {
    setCreatedDateFilter({ startDate: start, endDate: end });
  }
  ...
  useEffect(() => {
    fetchBanners();
  }, [currentPage, searchTerm, filterStatus, filterBannerType, createdDateFilter]);
